...
export class VisualAgent extends BaseAgent {
  ...
  async canHandle(message: string, _context: ConversationContext): Promise<number> {
    ...
    const visualKeywords = [
      'imagem', 'image', 'foto', 'photo', 'desenho', 'draw',
      'arte', 'art', 'visual', 'picture', 'ilustração', 'illustration',
      'gerar', 'generate', 'criar imagem', 'design', 'gráfico'
    ]
...
Response in Portuguese (Brazil) unless asked otherwise.`
  ...
  protected getKeywords(): string[] {
    return [
      'imagem', 'foto', 'desenho', 'arte', 'visual',
      'picture', 'ilustração', 'gerar', 'criar imagem'
    ]
  }
}